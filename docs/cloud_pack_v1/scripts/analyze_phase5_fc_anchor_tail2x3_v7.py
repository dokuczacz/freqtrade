...
MANIFEST = Path("user_data/phase5_fc_anchor_tail2x3_v7_manifest.json")
EXECUTION = Path("user_data/backtest_results/phase5_fc_anchor_tail2x3_v7_execution_results.json")
OUT_SUMMARY = Path("user_data/backtest_results/phase5_fc_anchor_tail2x3_v7_summary.json")
OUT_TABLE = Path("user_data/backtest_results/phase5_fc_anchor_tail2x3_v7_table.csv")
OUT_FAILURE_MODES = Path("user_data/backtest_results/phase5_fc_anchor_tail2x3_v7_failure_modes_top3.json")
RESULTS_ROOT = Path("user_data/backtest_results/phase5_fc_anchor_tail2x3_v7")
TARGET_WINDOWS = ["W002", "W012"]
REQUIRED_ECONOMIC_VALID_RATIO = 1.0
MAX_SOFT_PROFIT_DEGRADATION = -0.03
...
def main() -> None:
    ...
    family_stop = not soft_tail_nonpositive_all

    decision = "TAIL_MITIGATION_PASS_READY_FOR_ECONOMIC_RECHECK" if pass_ready else "TAIL_GUARD_LOGIC_REWORK_REQUIRED"
    if family_stop:
        decision = "GUARD_FAMILY_TERMINATED_NEW_HYPOTHESIS_REQUIRED"
    ...
    if not economic_valid_ratio_ok:
        ...
        fail_state = "TAIL_GUARD_FAMILY_TERMINATED_NEW_HYPOTHESIS_REQUIRED"
    ...
    summary = {
        ...
        "guard_family_stop": family_stop,
        "next_action_if_family_stop": "new_hypothesis_required" if family_stop else "continue_soft_axis",
        "gate": {
            "status": "PASS" if pass_ready else "FAIL",
            "rule": "activation_total>0 AND force_on_activations>0 AND SOFT delta_tail<=0 in W002/W012 AND SOFT delta_profit>=-0.03 in W002/W012",
