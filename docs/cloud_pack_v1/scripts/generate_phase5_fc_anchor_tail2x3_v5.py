...
MANIFEST_OUT = Path("user_data/phase5_fc_anchor_tail2x3_v5_manifest.json")
...
def main() -> None:
    ...
    if str(reconcile.get("effective_state", "")) != "tail2x3_v3":
        raise RuntimeError("effective_state is not tail2x3_v3 - reconcile required before V5")
    ...
    for win in TARGET_WINDOWS:
        ...
        for variant_label, variant_overrides in variants_for_run:
            run = f"FC-ANCHOR-TAIL2x3V5-{win}-{variant_label}"
            ...
            if variant_label == "VOL_SHOCK_SOFT":
                overrides["vol_shock_kill_enable"] = True
                overrides["vol_shock_atrp_mult"] = 1.4
                overrides["vol_shock_adx_min"] = 15
                overrides["vol_shock_lookback_bars"] = 8
                overrides["vol_shock_loss_floor_pct"] = -0.01
                overrides["negative_exit_cut_enable"] = False
    ...
    payload = {
        ...
        "purpose": "V5 minimal tail test: activation + impact with FORCE_ON vs SOFT on W002/W012",
        "execution_mode": "lean",
        "stage": "ANCHOR_TAIL2x3_V5",
        "planning_gate": {
            "objective": "Validate volatility shock guard wiring (FORCE_ON) and impact profile (SOFT).",
            "acceptance": "vol_shock_kill_activations_total>0 AND SOFT delta_tail_vs_ctrl<=0 in W002/W012 AND SOFT delta_profit_vs_ctrl>=-0.10 in W002/W012; FORCE_ON activations==0 => GUARD_WIRING_OR_CONTEXT_ISSUE",
            "shadow_ml_mode": "telemetry-only; no impact on entry/exit decisions",
            "required_artifacts": {
                "manifest": "user_data/phase5_fc_anchor_tail2x3_v5_manifest.json",
                "execution": "user_data/backtest_results/phase5_fc_anchor_tail2x3_v5_execution_results.json",
                "summary": "user_data/backtest_results/phase5_fc_anchor_tail2x3_v5_summary.json",
                "table": "user_data/backtest_results/phase5_fc_anchor_tail2x3_v5_table.csv",
                "failure_modes_top3": "user_data/backtest_results/phase5_fc_anchor_tail2x3_v5_failure_modes_top3.json",
                "shadow_summary": "user_data/backtest_results/phase5_fc_anchor_tail2x3_v5_shadow_ml_summary.json"
            }
        },
        "matrix": {
