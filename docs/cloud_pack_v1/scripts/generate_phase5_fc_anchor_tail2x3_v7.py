...
MANIFEST_OUT = Path("user_data/phase5_fc_anchor_tail2x3_v7_manifest.json")
...
def main() -> None:
    ...
    if str(reconcile.get("effective_state", "")) != "tail2x3_v3":
        raise RuntimeError("effective_state is not tail2x3_v3 - reconcile required before V7")
    ...
    for win in TARGET_WINDOWS:
        ...
        for variant_label, variant_overrides in variants_for_run:
            run = f"FC-ANCHOR-TAIL2x3V7-{win}-{variant_label}"
            ...
            if variant_label == "VOL_SHOCK_SOFT":
                overrides["vol_shock_kill_enable"] = True
                overrides["vol_shock_atrp_mult"] = 0.85
                overrides["vol_shock_adx_min"] = 8
                overrides["vol_shock_lookback_bars"] = 3
                overrides["vol_shock_loss_floor_pct"] = -0.002
                overrides["negative_exit_cut_enable"] = False
    ...
    payload = {
        ...
        "purpose": "V7 minimal tail-reduction with hard stop contract (W002/W012)",
        "execution_mode": "lean",
        "stage": "ANCHOR_TAIL2x3_V7",
        "planning_gate": {
            "objective": "Loss-side early SOFT cut only, while preserving activation contract.",
            "acceptance": "PASS only if SOFT delta_tail_vs_ctrl<=0 in W002/W012 AND SOFT delta_profit_vs_ctrl>=-0.03 in W002/W012 and activation contract holds",
            "shadow_ml_mode": "telemetry-only; coverage denominator uses emitted entries",
            "required_artifacts": {
                "manifest": "user_data/phase5_fc_anchor_tail2x3_v7_manifest.json",
                "execution": "user_data/backtest_results/phase5_fc_anchor_tail2x3_v7_execution_results.json",
                "summary": "user_data/backtest_results/phase5_fc_anchor_tail2x3_v7_summary.json",
                "table": "user_data/backtest_results/phase5_fc_anchor_tail2x3_v7_table.csv",
                "failure_modes_top3": "user_data/backtest_results/phase5_fc_anchor_tail2x3_v7_failure_modes_top3.json",
                "shadow_summary": "user_data/backtest_results/phase5_fc_anchor_tail2x3_v7_shadow_ml_summary.json"
            },
            "failure_states": {
                "wiring_or_context": "FORCE_ON activations == 0",
                "tail_impact_fail": "SOFT delta_tail_vs_ctrl > 0 in any target window",
                "profit_floor_fail": "SOFT delta_profit_vs_ctrl < -0.03 in any target window",
                "family_stop": "if V7 still has positive SOFT tail, terminate this guard family"
            }
        },
        "matrix": {
